...
const Header = () => {
  ...
  return (
    <header>
        ...
        <div className='hidden xl:block'>
          <ul className='flex flex-row gap-5 items-center justify-center h-full'>
            ...
            <li onMouseEnter={() => setLi(true)} onMouseLeave={() => setLi(false)} className={`group hover:text-black text-gray relative`}>
              <button className='flex flex-row items-center'>{t("header.bottomHeader.about")}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"
                  fill="currentColor"
              ...
              <NavLink to={"/certificate"} className={`${li === true ? "scale-y-100" : "scale-0"} absolute -bottom-10 rounded-md -right-4 -left-4 shadow-md py-2 px-4 flex flex-row items-center justify-center duration-300`}>
                <button>
                  {t("header.bottomHeader.certificate")}
                </button>
              </NavLink>
            </li>
      ...
      <div className={`absolute top-0 bg-white py-10 w-1/3 z-20 h-full flex flex-col items-center ${openNav ? "translate-x-0" : "-translate-x-full"} duration-300`}>
        ...
        <ul className='flex flex-col gap-5 items-start justify-center w-10/12 pt-10'>
          ...
          <Accordion open={open === 1} icon={<Icon id={1} open={open} />}>
            <AccordionHeader className='border-b-0 text-green text-base font-normal py-0 px-0 justify-start hover:text-green' onClick={() => handleOpen(1)}>{t("header.bottomHeader.about")}</AccordionHeader>
